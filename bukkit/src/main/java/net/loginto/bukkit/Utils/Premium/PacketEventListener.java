    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (packetType == PacketType.Login.Client.LOGIN_START) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                ...
                boolean premiumTableContainsPlayer = database.premiumTableContainsPlayer(pName);
                boolean isPremium = database.isPremium(pName);

                //If the res code is not 429 or 200, the player is cracked.
                if (mojangNameApiResCode != 200 || (!isPremium && premiumTableContainsPlayer)) {
                    ...
                    if (!premiumTableContainsPlayer) {
                        database.updatePlayerAccountStatus(pName, false);
                    }
                    return;
                }
                ...
                try {
                    ...
                    Bukkit.getScheduler().runTask(plugin, () -> {
                        if (status != 200) {
                            ...
                            if (!database.premiumTableContainsPlayer(userTarget.name)) {
                                database.updatePlayerAccountStatus(userTarget.name, true);
                            }
                        }
